...
#define MPU6050                     // 定义使用的传感器为MPU6050
#define MOTION_DRIVER_TARGET_MSP430 // 运动驱动目标平台,原为MSP430(现改为STM32F1)
...
uint8_t mpu_dmp_init(void)
{
    ...
    if (mpu_init() == 0) // 初始化MPU6050
    {
        ...
        if (res)
            return 5;
        res = dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT | DMP_FEATURE_TAP | // 使能dmp功能
...
// 得到dmp处理后的数据(注意,本函数需要比较多栈,局部变量多的)
// pitch:俯仰角 精度:0.1度   范围:-90.0度 <---> +90.0度
// roll:横滚角  精度:0.1度   范围:-180.0度<---> +180.0度
// yaw:航向角   精度:0.1度   范围:-180.0度<---> +180.0度
// 返回值:0,成功
//     其他,失败
uint8_t mpu_dmp_get_data(float *pitch, float *roll, float *yaw)
{
    ...
    unsigned char more;
    long quat[4];
    // 外部全局变量（在 pid.c 定义），用于直立环/转向环的角速度项
    extern short gyrox, gyroy, gyroz;
    if (dmp_read_fifo(gyro, accel, quat, &sensor_timestamp, &sensors, &more))
        return 1;
    /* 取出 DMP 解算出的陀螺仪原始角速度（chip frame），赋给全局变量。
       注意：陀螺仪满量程 ±2000dps，raw 需除 16.4 才是 dps。 */
    if (sensors & INV_XYZ_GYRO)
    {
        gyrox = gyro[0];
        gyroy = gyro[1];
